-- ✅ CORRECCIÓN: Habilitar Respuestas Anidadas - YoMinero
-- USAR TABLA CORRECTA: post_responses (NO responses)
...
CREATE INDEX IF NOT EXISTS idx_post_responses_parent_response_id 
ON post_responses(parent_response_id);

-- 3. Crear índice compuesto para consultas complejas  
CREATE INDEX IF NOT EXISTS idx_post_responses_post_parent 
ON post_responses(post_id, parent_response_id);

-- 4. Función RPC para crear respuestas anidadas (CORREGIDA)
...
-- 6. Función mejorada para obtener respuestas con estructura anidada (CORREGIDA)
...
COMMENT ON COLUMN post_responses.parent_response_id IS 'ID de respuesta padre para respuestas anidadas. NULL para respuestas de primer nivel.';
...
-- Ahora tu aplicación Flutter podrá usar respuestas anidadas completamente